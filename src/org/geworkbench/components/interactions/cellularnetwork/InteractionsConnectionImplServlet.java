...
public class InteractionsConnectionImplServlet extends HttpServlet {
		...
		try {
            ...
            if (needAuthentication(tokens, conn, req))
				...
				String sql = getSqlString(tokens, conn);			 
				 ...
				 // if (logger.isDebugEnabled()) 
				//	  logger .debug("doPost(HttpServletRequest, HttpServletResponse) - InteractionsConnectionImplServlet doPost, sql string is " + sql );  
	...
	private boolean needAuthentication(String[] tokens, Connection conn,
	...
	private String getSqlString(String[] tokens, Connection conn)
			throws SQLException {
		String aSql = null;
		String methodName = tokens[1];
		if (methodName.equalsIgnoreCase(GET_PAIRWISE_INTERACTION)) {
	...
	private String getAuthentication(Connection conn, String context,
